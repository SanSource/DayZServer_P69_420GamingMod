...
if (isServer) then
{
	...
	{
		DZ_ServerBrain = [] execFSM "\dz\420Mods\PVP_scripts_coastal\server\scripts\fsm\server_taskmanager.fsm";
	};
...
//oldui 				hintSilent _str;
	...
	(owner _agent) SpawnForClient {
		...
		cus_killed_EH = [player,"NIGGER DIED!"];
		publicVariableServer "cus_killed_EH";
	};
