...
_BandAmmo = ["G_GrenadeHand","G_Grenade","LaserBombCore","FuelExplosion"];
_niggefr = format ["%1",(getPlayerUID _source)];
_namemat = format ["%1",(name _source)];

if ( _ammo in _BandAmmo ) exitWith
{
diag_log format ["**IMPORTANT DAMAGE-LEAK** Hit:[%1] Damage:[%2] Ammo:[%4] SourceName[%5] UID[%6]",_selection,_damage,_source,_ammo,_namemat,_niggefr];
0; ProfileStop "event_assessDamage.sqf";
};


if ( _niggefr == "" and !(isPlayer _source) and ( _ammo != "MeleeZombie" ) and ( _ammo != "MeleeWolf" ) ) exitWith
{
diag_log format ["**IMPORTANT DAMAGE-LEAK UNKOWN SOURCE!!** Hit:[%1] Damage:[%2] Ammo:[%4] SourceName[%5] UID[%6]",_selection,_damage,_source,_ammo,_namemat,_niggefr];
0; ProfileStop "event_assessDamage.sqf";
};

_sourceLifestate = lifeState _source;
if (_sourceLifestate == "ALIVE") then {
_ammoConfig = configFile >> "cfgAmmo" >> _ammo;
_isMelee = _ammo isKindOf "MeleeDamage";
_isZombie = _source isKindOf "ZombieBase";
_final = 0;
_ffmat = format ["suicide"];

if (_isZombie) then {
	_ffmat = format ["zombie"];
   _agent setVariable ["prevDamage",_ffmat];
};

if (isPlayer _source) then {
	_ffmat = format ["%1",(getPlayerUID _source)];
	_agent setVariable ["prevDamage",_ffmat];
};
...
if ( isPlayer _source && isPlayer _agent ) then
{
	...
	else
	{
		...
		if (_bone == "head") then { _agent setVariable ["HSD",1]; };
