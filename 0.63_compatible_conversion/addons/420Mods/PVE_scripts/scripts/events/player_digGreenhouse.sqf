...
switch (_groundType) do
{
	...
	case("GardenPlot"): {
		...
		if (isNil "_baseFertility") then {
			...
			hint format["Warning! Object %1 has undefined value baseFertility! Setting it to %2 now.", _greenhouse, _baseFertility];
...
fnc_digSlot = {
	...
	_user setSlotLock ['Hands', false];
	diag_log format ['Geenhouse:::Lock::: %1', _user getSlotLock 'Hands'];
	
	if ((vehicle _user) distance [10060.891, 5436.0332] < 500)
	exitWith{
	[_user, "I can not dig a plot here, too close to the safezone!", "colorImportant"] call fnc_playerMessage;
	};

	if ((vehicle _user) distance [2314.9932, 9617.6377] < 500)
	exitWith{
	[_user, "I can not dig a plot here, too close to a trader!", "colorImportant"] call fnc_playerMessage;
	};

	if ((vehicle _user) distance [6359.7725, 7760.9819] < 500)
		exitWith{
		[_user, "I can not dig a plot here, too close to a trader!", "colorImportant"] call fnc_playerMessage;
	};
...
{	
	...
	if ( _freeSlotId > -1 ) then
	{
		...
		diag_log format ['Geenhouse:::Lock::: %1', _person getSlotLock 'Hands'];
	}
	else
	{
