class MissionServer2 extends MissionServer
{
	...
	void OnInit()
	{
		if (m_EnfTraderMod)
		{
			GetGame().ExecuteSQF("call compile preprocessFileLineNumbers '\\dz\\420Mods\\PVE_subscripts\\traderScript.sqf';");
			GetGame().ExecuteSQF("call compile preprocessFileLineNumbers '\\dz\\420Mods\\PVE_subscripts\\Trader_objs.sqf';");
			GetGame().ExecuteSQF("diag_log 'MISSION: Script exec....traderScript.sqf';");
			GetGame().ExecuteSQF("diag_log 'MISSION: Script exec....Trader_objs.sqf';");
		}
		
		if (m_EnfVIPsystem)
		{
			GetGame().ExecuteSQF("call compile preprocessFileLineNumbers '\\dz\\420Mods\\PVE_subscripts\\VIP_CoolDownSYS.sqf';");
			GetGame().ExecuteSQF("diag_log 'MISSION: Script exec....VIP_CoolDownSYS.sqf';");
		}
		
		if (m_CustomBuilds)
		{
			GetGame().ExecuteSQF("call compile preprocessFileLineNumbers '\\dz\\420Mods\\PVE_subscripts\\Builds_0.sqf';");
			GetGame().ExecuteSQF("call compile preprocessFileLineNumbers '\\dz\\420Mods\\PVE_subscripts\\Builds_1.sqf';");
			GetGame().ExecuteSQF("diag_log 'MISSION: Script exec....Builds_0.sqf';");
			GetGame().ExecuteSQF("diag_log 'MISSION: Script exec....Builds_1.sqf';");
		}
		
		if (m_AdminTool)
		{
			GetGame().ExecuteSQF("call compile preprocessFileLineNumbers '\\dz\\420Mods\\PVE_subscripts\\Admin_Tool.sqf';");
			GetGame().ExecuteSQF("diag_log 'MISSION: Script exec....Admin_Tool.sqf';");
		}
		

	   FileHandle AdminList = OpenFile("$profile:" + "Admin_List" + ".txt", FileMode.READ);
	   if (AdminList != 0)
		{
			GetGame().ExecuteSQF("Owners = [];");
			string line_content;
			while ( FGets( AdminList, line_content ) > 0 )
			{
				Admin_UIDS.Insert( line_content );
				GetGame().ExecuteSQF(" Owners = Owners + ['"+ line_content +"']; ");
				Print("Admin: "+line_content+" Processing UID..");
			}
